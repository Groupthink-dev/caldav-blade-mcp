...
Bearer token auth for remote/tunnel access. Set ``CALDAV_MCP_API_TOKEN`` env var.
...
def get_bearer_token() -> str | None:
    ...
    token = os.environ.get("CALDAV_MCP_API_TOKEN", "").strip()
...
class BearerAuthMiddleware:
    ...
    When ``CALDAV_MCP_API_TOKEN`` is set, every request must carry a matching
