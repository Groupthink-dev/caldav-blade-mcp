...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live CalDAV)"
...
test-e2e:
	CALDAV_E2E=1 uv run pytest tests/e2e/ -v -m e2e

test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/caldav_blade_mcp --cov-report=term-missing -v
...
type-check:
	uv run mypy src/caldav_blade_mcp
